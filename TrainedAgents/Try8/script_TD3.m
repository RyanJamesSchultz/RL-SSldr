...
criticOptions = rlOptimizerOptions('LearnRate',5e-5,'GradientThreshold',1,'L2RegularizationFactor',1e-4);
actorOptions  = rlOptimizerOptions('LearnRate',5e-5,'GradientThreshold',1,'L2RegularizationFactor',1e-4);
...
agentOptions = rlTD3AgentOptions('SampleTime',1,...
    ...
    'TargetSmoothFactor',5e-5,...
    'TargetUpdateFrequency',12,...
    'DiscountFactor',0.99);
agentOptions.ExplorationModel.StandardDeviation = [0.0;0.05];
agentOptions.ExplorationModel.StandardDeviationDecayRate = [0;5e-6];
agentOptions.ExplorationModel.StandardDeviationMin = [0;1e-2];
agentOptions.TargetPolicySmoothModel.StandardDeviation = [0.0;0.10];
agentOptions.TargetPolicySmoothModel.StandardDeviationDecayRate = [0;5e-6];
