function [Reward,End_Flag,End_type]=RewardFxn(env,SSc,SSp)
  ...
  Rvr=+2;   % Reward for staying within the relative-velocity range.
  Rva=+1;   % Reward for staying within the absolute-velocity range.
  ...
  % Reward for staying within a range of the target velocity.
  dlogV=log10(SSc.V(end))-log10(Vtarg);
  Reward=Reward+Rva*normpdf(abs(dlogV),0,dlogVaTA)*dlogVaTA*sqrt(2*pi());
