...
criticOptions = rlOptimizerOptions('LearnRate',2e-5,'GradientThreshold',1,'L2RegularizationFactor',1e-4);
...
% Modify agent noise model for plateau 2 training.
trainingOpts.MaxEpisodes=4e5;
agent.AgentOptions.ExplorationModel.StandardDeviation = [0.0;0.05];
agent.AgentOptions.ExplorationModel.StandardDeviationDecayRate = [0;1e-4];
agent.AgentOptions.ExplorationModel.StandardDeviationMin = [0;0.02];
agent.AgentOptions.TargetPolicySmoothModel.StandardDeviation = [0.0;0.10];
agent.AgentOptions.TargetPolicySmoothModel.StandardDeviationDecayRate = [0;1e-4];
agent.AgentOptions.TargetPolicySmoothModel.StandardDeviationMin = [0;0.02];

% Train again (on noise plateau 2).
trainingStats = train(agent,SS1env,trainingOpts);

% Save again.
SS1env.reset();
save('initialAgent_p2.mat','agent','trainingStats','SS1env');

% Modify agent noise model for plateau 3 training.
trainingOpts.MaxEpisodes=3e5;
agent.AgentOptions.ExplorationModel.StandardDeviation = [0.0;0.02];
agent.AgentOptions.ExplorationModel.StandardDeviationDecayRate = [0;1e-4];
...
% Train again (on noise plateau 3).
trainingStats = train(agent,SS1env,trainingOpts);

% Save again.
SS1env.reset();
save('initialAgent_p3.mat','agent','trainingStats','SS1env');

% % Modify agent noise model for plateau 4 training.
% trainingOpts.MaxEpisodes=2e5;
...
% agent.AgentOptions.TargetPolicySmoothModel.StandardDeviationDecayRate = [0;1e-4];
% agent.AgentOptions.TargetPolicySmoothModel.StandardDeviationMin = [0;0.005];
% 
% % Train again (on noise plateau 4).
% trainingStats = train(agent,SS1env,trainingOpts);
% 
% % Save again.
% SS1env.reset();
% save('initialAgent_p4.mat','agent','trainingStats','SS1env');
